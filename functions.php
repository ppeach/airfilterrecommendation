...
function getHepa($country, $sheets_id, $range){
...
function calculateACH($data, $ach, $titles=array(), $room_size=null, $rms_type=null, $no_of_occ=null){

    // Calculate ACH and Total Cost
    foreach($data as $key => $value){
        if($ach == 'ach'){
            if($rms_type == 'm3'){
                $ach_unit = (6 * $room_size)/$value[$titles[0]];
                $ach_value = (ceil($ach_unit) * $value[$titles[0]])/$room_size;
                $ach_value_min = ((ceil($ach_unit) - 1) * $value[$titles[0]])/$room_size;
            } else {
                $ach_unit = (6 * $room_size)/$value[$titles[1]]/60;
                $ach_value = (ceil($ach_unit) * $value[$titles[1]] * 60)/$room_size;
                $ach_value_min = ((ceil($ach_unit) - 1) * $value[$titles[1]] * 60)/$room_size;
            }
        } else {
            $ach_unit = (20 * $no_of_occ)/$value[$titles[2]];
            $ach_value = (ceil($ach_unit) * $value[$titles[2]])/$no_of_occ;
            $ach_value_min = ((ceil($ach_unit) - 1) * $value[$titles[2]])/$no_of_occ;
        }
        ...
        $data[$key]['Total Cost'] = $value[$titles[3]] * $ach_needs;
    }
